...
data.hist$comparison <- c(replicate(maxT+1,"freebayes_gatk"),
                          replicate(maxT+1,"freebayes_mpileup"),
                          replicate(maxT+1,"gatk_mpileup"))
...
save_as_png(print(p), file.name =paste0(outputs,"/facet_all.png"), width = 10, height = 4)
