...
graph3 <-Heatmap(heat_matrix, name = "frequency", 
                   col = col_fun,
                   heatmap_width = unit(35, "cm"), heatmap_height = unit(nrow(heat_matrix) +5, "cm"),
                   border = T,
                   #border_gp = gpar(col = "grey"),
                   rect_gp = gpar(col = "white", lwd = 1),
                   cluster_rows = FALSE,cluster_columns  = FALSE,
...
save_image(print(graph3),
           paste0(plots_folder, "/",specific_fp,".png"),
           width = 20, height = 12, res = 100)

output_table <- cbind(heat_matrix, 
                      new_annotations[, c("annot", "genes", "HGVS.c", "HGVS.p", "POS", "mutation")])

write.csv(output_table, 
          paste0(plots_folder, "/",specific_fp,".csv"), 
          row.names = FALSE)

rm(graph3, extra, input_data, aa_abbreviations, 
   output_table, mutations_source, i, specific_lineage,
   timepoints)
