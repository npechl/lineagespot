...
temp = decisions %>% group_by(V1) %>% summarise(V2 = paste(V2, collapse = ","))

one.run = function(x) {
  
  r.split = stringr::str_split(x[2], ",", simplify = TRUE)
  r.split = as.vector(r.split)
  
  r.split = unique(r.split)
  
  out = data.table::data.table(V1 = x[1],
                               V2 = paste(r.split, collapse = ","))
  
}

temp = apply(temp, 1, one.run)
...
one.run = function(x, expr, AD, DP, source) {
  ...
  stats = data.table::data.table(total.overlap = base::length(expr),
                                 ...
                                 overlap.rules = paste(expr.var, collapse = ","),
...
out$overlap.rules = rules.stats$overlap.rules
...
base::colnames(out) = base::c("Lineage", "Rules", 
                              "Tree.Overlap", "Total.Overlap", "Total.Overlap.Var", "Overlap.Rules", "Total",
...
utils::write.table(out, 
                   paste(stringr::str_replace(vcf.path, ".vcf", "-lineagespot-overrules-v2.tsv"), sep = ""), 
                   row.names = FALSE, 
                   quote = FALSE, 
                   sep = "\t")
